...
## Android
Android SDK의 GCCS 인증 기능 구현 방법을 안내합니다.
...
|Key|Value|Description|
|------|---|---|
|isAuth|Boolean|추가 인증에 성공 한 경우 true, 실패한 경우 false|
...
## 인증 결과
`GuardianSdk` 의 `resultAuth()`로 API를 호출합니다.   
요청 성공 시 토큰을 받습니다.
...
## 인증 취소
`GuardianSdk`의 `cancelAuth()`로 API를 호출합니다.
